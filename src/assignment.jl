...
create_param_array(N::SInt; ncmp::Int=3) = (s = (N.+1).data; zeros(ComplexF, s..., ncmp, ncmp))  # (i,j,...) element is ncmp×ncmp tensor
...
function assign_param!(paramKd::AbsArrComplexF{K₊₂},  # electric (magnetic) parameter array to set; K₊₂ = K+2, where 2 is rank of material tensor
                       ...
                       pind2matprm::AbsVec{S²ComplexF{Kf,Kf²}},  # map from pind to electric (magnetic) material parameters; Kf² = Kf^2
    ...
    for nw = 1:Kf⏐₁  # if Kf⏐₁ == Kf for Kf = 3, w = X̂, Ŷ, Ẑ
        ...
        #
        # I thought I could replace the condition Kf⏐₁==1 with isfield_ortho_shape(Kf,K),
        # because gt_cmp becomes gt₀ when isfield_ortho_shape(Kf,K) = true.  The replacement
        # is incorrect, however, because we need gt_cmp to become gt₀ even when the shape
        # and field subspaces coincide (e.g., K = Kf = 3) to assign the off-diagonal entries
        # of the material parameter tensors at voxel corners.
...
# This is the innermost function that is customized for MaxwellBase (i.e., it takes arrays
...
#
# For Kf⏐₁ == Kf.
assign_param_cmp!(param_cmp::AbsArrComplexF{K},  # output material parameter array (array of scalars)
                  ...
                  pind2matprm::AbsVecComplexF,  #  input map from pind to material parameter
...
# For Kf⏐₁ == 1.
# Below, note that the element type of pint2matprm, S²ComplexF{Kf,Kf²}, is always a matrix
# (i.e., rank-2 tensor), regardless of the value of Kf.  For example, if we are assigning μz
# in a 2D TE problem, μz is a type of S²ComplexF{1,1}.  This is why K₊₂ is K + 2 regardless
# of the value of Kf, where 2 is the rank of the material parameter tensor.
assign_param_cmp!(param_cmp::AbsArrComplexF{K₊₂},  # output material parameter array (array of tensors); M = K+2
                  oind′_cmp::AbsArr{ObjInd,K},  # output object index array
                  oind2shp::AbsVec{Shape{K,K²}},  # input map from oind to shape
                  oind2pind::AbsVec{ParamInd},  # input map from oind to pind
                  pind2matprm::AbsVec{S²ComplexF{Kf,Kf²}},  #  input map from pind to material parameter
...
function assign_val_shape_impl!(array::AbsArr{T},
    ...
    bn, bp = bounds(shape)  # (SVec{K}, SVec{K})
